...
# Bu fonksiyon, ağın toplam giriğini hesaplar.
def net_hesapla(i):
    NET = a1[i]*agirlik[0]+a2[i]*agirlik[1]+a3[i]*agirlik[2]+a4[i]*agirlik[3];
    return NET

# Bu fonksiyon, perceptron'un çıkışını belirler. Eğer net değer eşik değerinden büyükse çıkış 1, değilse 0 olur.
...
# Bu fonksiyon, ağırlıkları günceller. Eğer beklenen çıkış (cikti[i]) ile gerçek çıkış (o) birbirinden farklıysa ağırlıklar güncellenir.
...
#Bu kısım, eğitim döngüsünü başlatır. Her bir iterasyonda, giriş verilerinin ağ üzerindeki etkisini değerlendirir, perceptron çıkışını alır ve ağırlıkları günceller.
for i in range(1):
